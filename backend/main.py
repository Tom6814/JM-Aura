...
async def on_startup():
    from backend.core.db.database import engine, Base
    Base.metadata.create_all(bind=engine)
    
@app.exception_handler(Exception)
async def global_exception_handler(request: Request, exc: Exception):
    print(f"[Global Error] Unhandled exception on {request.url.path}: {exc}")
    traceback.print_exc()
    return JSONResponse(
        status_code=500,
        content={"st": 500, "msg": "Internal Server Error", "detail": str(exc)}
    )
...
def _relogin_from_saved_config(user: str | None = None) -> bool:
    u, p = _get_saved_jm_credentials(user=user)
    if not u or not p:
        return False
    try:
        data = LoginReq2(u, p).execute()
...
@app.delete("/api/v2/{source}/download/tasks/{task_id}")
def v2_cancel_download_task(source: str, task_id: str):
    try:
        if source == "jm":
            success = download_task_manager.cancel_task(task_id)
            if success:
                return _v2_ok({"status": "cancelled"})
            else:
                raise ProviderError("Task not found or cannot be cancelled", status=400)
        raise ProviderError("Unknown source", status=400)
    except Exception as e:
        return _v2_err(e)
...
if __name__ == "__main__":
    ...
    for attempt in range(max_retries):
        try:
            print(f"[Aura Server] Starting server on {host}:{port} (Attempt {attempt + 1}/{max_retries})")
            uvicorn.run(app, host=host, port=port)
            break  # 正常退出
        except Exception as e:
            print(f"[Aura Server] CRITICAL ERROR: Server crashed: {e}")
            traceback.print_exc()
            if attempt < max_retries - 1:
                print(f"[Aura Server] Restarting in {retry_delay} seconds...")
                time.sleep(retry_delay)
            else:
                print("[Aura Server] Max retries reached. Shutting down.")
                sys.exit(1)
