...
const BookCard = {
    ...
    methods: {
        ...
        getAuthor() {
            return this.book.author || this.book.writer || '';
        },
        getId() {
...
createApp({
    ...
    data() {
        return {
            ...
            jmSelectedSubCategory: '',
            jmSelectedSubCategoryTitle: '全部',
            jmCategoryKeyword: '',
            jmCategoryItems: [],
    ...
    computed: {
        ...
        jmSelectedCategoryObj() {
            const id = String(this.jmSelectedCategory || '0');
            const list = Array.isArray(this.jmCategories) ? this.jmCategories : [];
            return list.find(c => String(this.getJmCategoryId(c)) === id) || null;
        },
        jmSubCategories() {
            const c = this.jmSelectedCategoryObj;
            const subs = c && typeof c === 'object' ? c.sub_categories : null;
    ...
    methods: {
        ...
        selectJmCategory(c) {
            ...
            this.jmSelectedSubCategory = '';
            this.jmSelectedSubCategoryTitle = '全部';
        ...
        setJmSubCategory(sc) {
            if (!sc) {
                this.jmSelectedSubCategory = '';
                this.jmSelectedSubCategoryTitle = '全部';
                this.loadJmCategoryList(this.jmSelectedCategory, 1);
                return;
            }
            if (typeof sc === 'string') {
                this.jmSelectedSubCategory = sc;
                this.jmSelectedSubCategoryTitle = sc;
                this.loadJmCategoryList(this.jmSelectedCategory, 1);
                return;
            }
            const slug = sc.slug || sc.SLUG || '';
            const name = sc.name || sc.title || sc.CNAME || '';
            this.jmSelectedSubCategory = String(slug || '');
            this.jmSelectedSubCategoryTitle = String(name || slug || '子分类');
            this.loadJmCategoryList(this.jmSelectedCategory, 1);
        },
        ...
        async loadJmCategoryList(categoryId, page = 1) {
            ...
            try {
                const sub = String(this.jmSelectedSubCategory || '').trim();
                const tagQ = sub ? `&tag=${encodeURIComponent(sub)}` : '';
                const res = await fetch(`/api/v2/jm/leaderboard?category=${encodeURIComponent(cat)}&page=${p}&sort=tf${tagQ}`);
                const json = await res.json().catch(() => ({}));
                if (!res.ok || (json.st && json.st !== 1001)) {
                    throw new Error(json.msg || json.detail || 'Failed');
                }
        ...
        toggleFavorite(album) {
            ...
            (async () => {
                try {
                    ...
                    this.favoriteStateById = { ...(this.favoriteStateById || {}), [aid]: desired };
                    if (this.selectedAlbum && String(this.selectedAlbum.album_id) === aid) {
                        this.selectedAlbum.is_favorite = desired;
                    }
                    if (this.currentTab === 'jm_favorites' && !desired) {
                        this.jmFavItems = (this.jmFavItems || []).filter(x => String(x && x.album_id ? x.album_id : '') !== aid);
                    }
                    this.showToast(desired ? '已收藏' : '已取消收藏', 'success');
                } catch (e) {
