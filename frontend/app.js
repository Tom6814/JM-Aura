...
const BookCard = {
    ...
    template: `
    <div class="md-card overflow-hidden cursor-pointer group flex flex-col h-full transition-all duration-300"
         @click="handleClick">
        <div class="aspect-[3/4] overflow-hidden relative bg-surface-variant">
            <img :src="getCover()" class="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105" loading="lazy" alt="Cover">
            <div class="absolute inset-0 bg-black/5 opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
            
            <div class="absolute top-2 right-2 flex flex-col items-end gap-1">
                 <span v-if="book.status" class="px-2 py-0.5 rounded-md bg-black/60 text-[10px] text-white font-medium shadow-sm">
                    {{ book.status }}
                 </span>
            </div>
        </div>
        <div class="p-3 md:p-4 flex flex-col flex-1">
            <h3 class="font-bold text-sm md:text-base line-clamp-2 leading-tight mb-1 group-hover:text-primary transition-colors" :title="getTitle()">
                {{ getTitle() }}
            </h3>
